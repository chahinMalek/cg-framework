...
def new_edges(t_points: List[Point], t_edges: List[LineSegment], point: Point):
    new_edges = []
...
def find_pair_edge(triangulation, edge):
    faces = []
...
def flip_edges(triangulation, edges_to_flip):

    for edge in edges_to_flip:
        flip_edge(triangulation, edge)


def flip_edge(triangulation, edge):
...
def add_triangles(triangulation, point, tri_points):
    tri_edges = triangulation.get_edges()
    new_edges_ = new_edges(tri_points, tri_edges, point)
    edges_to_flip = []
    for edge_1 in new_edges_:
        for edge_2 in new_edges_:
            if edge_1 == edge_2: 
                break
            tri_edge = LineSegment(edge_1.second, edge_2.second)

            if tri_edge in tri_edges:
                tri_edge = tri_edges[tri_edges.index(tri_edge)]
            elif tri_edge.reversed() in tri_edges:
                tri_edge = tri_edges[tri_edges.index(tri_edge.reversed())]
            else:
                continue
